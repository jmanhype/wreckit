...
async function testCircuitBreakerMetrics() {
  ...
  try {
    ...
    if (response.status() !== 200) {
      throw new Error(
        `Prometheus endpoint returned status ${response.status()}`,
      );
    }
    ...
    console.log(
      `📈 Retrieved ${metricsText.length} characters of metrics data`,
    );
    ...
    const healthScoreMatch = metricsText.match(
      /cyb_circuit_breaker_health_score{[^}]+} ([\d.]+)/,
    );
    if (healthScoreMatch) {
      console.log(`🔹 Health Score: ${healthScoreMatch[1]}`);
    }

    const stateMatch = metricsText.match(
      /cyb_circuit_breaker_state{[^}]+} ([\d.]+)/,
    );
    if (stateMatch) {
      const stateValue = parseFloat(stateMatch[1]);
      const stateText =
        stateValue === 0 ? "closed" : stateValue === 1 ? "open" : "half_open";
      console.log(`🔹 Circuit State: ${stateText} (${stateValue})`);
    }

    const totalCountMatch = metricsText.match(
      /cybernetic_health_circuit_breakers_total_count ([\d.]+)/,
    );
    if (totalCountMatch) {
    ...
    console.log(
      `✅ Circuit breaker metrics found: ${foundMetrics.length}/${circuitBreakerMetrics.length}`,
    );
    console.log(
      `✅ Provider metrics found: ${providerMetrics.length}/${providers.length}`,
    );
